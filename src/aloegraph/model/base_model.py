"""
- **Module:** `src/aloegraph/model/base_model.py`
...
class AloeEdge(BaseModel):
    ...
    completion_check: Optional[Callable[[], tuple[bool, object, Optional[str]]]] = Field(
        default=None, exclude=True)
    completion_check_retries: Optional[int] = 5,
    transition_checks: list[Callable[[object], bool]] = Field(
...
class Refinement(BaseModel):
    id: Optional[str] = Field(
        default_factory=str, description="unique id for this refinement")
    source: str = Field(description="user or agent suggested")
    operation: Literal["add", "modify", "delete", "commit"]
    refinement_target_type: Literal["requirement",
                                    "generation_step", "validation_step", "refinement"]
    refinement_target_id: Optional[str] = Field(
        default_factory=str, description="id of item for suggested refinement")
    refinables_covered: list[str] = Field(
        default_factory=list, description="id of refinables covered by this refinement")
    text: Optional[str] = Field(description="suggested refinement")
    ordinal: Optional[float] = -1
    user_comment: Optional[str] = Field(
        description="user can request specific modification from agent")
